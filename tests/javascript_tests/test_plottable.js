requirejs(['jquery', 'model'], function($, model) {
  $(document).ready(function() {
   ...
   QUnit.test('Test toString method', function(assert) {
      ...
      var plot = new Plottable('foo', ['a', 'b', 'c'], [0.2, 0.3, 0.5, 0.2], 1);
     assert.equal(plot.toString(),
                  'Sample: foo located at: (0.2, 0.3, 0.5, 0.2) metadata: ' +
                  '[a, b, c] at index: 1 and without confidence intervals.',
     ...
     assert.equal(plot.toString(),
                  'Sample: foo located at: (0.2, 0.3, 0.5, 0.2) metadata: ' +
                  '[a, b, c] at index: 1 and with confidence intervals at ' +
                  '(0.2, 0.05, 0.01, 0.2).',
                  'Test correctly converted Plottable to string type with ' +
                  'confidence intervals.');

      plot = new Plottable('foo', ['a', 'b', 'c'], [0.2, 0.3, 0.5, 0.2]);
     assert.equal(plot.toString(),
                  'Sample: foo located at: (0.2, 0.3, 0.5, 0.2) metadata: ' +
                  '[a, b, c] without index and without confidence intervals.',
                  'Test correctly converted Plottable to string type without ' +
                  'index.');
    });
  });
});
