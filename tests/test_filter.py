...
class TopLevelTests(TestCase):
    ...
    def setUp(self):
        self.jk_coords_header = [['1', '2', '3'],
                                 ['1', '2', '3'],
                                 ['1', '2', '3'],
                                 ['1', '2', '3']]
        self.jk_coords_data = [array([[1.2, 0.1, -1.2],
                                      [-2.5, -4.0, 4.5],
                                      [0.11, 5.33, -0.23]]),
                               array([[-1.4, 0.05, 1.3],
                                      [2.6, 4.1, -4.7],
    ...
    def test_filter_Samples_from_coords(self):
        ...
        with self.assertRaises(ValueError):
            filter_samples_from_coords(self.coords_header, self.coords_data,
                                       ['foo', 'bar', 'PC.666'])

        # check it keeps the requested samples
        out_h, out_c = filter_samples_from_coords(self.coords_header,
                                                  self.coords_data,
                                                  ['PC.636', 'PC.355'])
        self.assertEquals(out_h, ['PC.355', 'PC.636'])
        assert_almost_equal(out_c, array([[-0.2, -0.1, 0.06, -0.06],
                                          [0.1, -0.1, -0.2, 0.08]]))
        ...
        out_h, out_c = filter_samples_from_coords(self.coords_header,
                                                  ...
                                                  ['PC.636', 'PC.355'],
                                                  negate=True)
        self.assertEquals(out_h, ['PC.635', 'PC.354'])
        assert_almost_equal(out_c, array([[-0.3, 0.04, -0.1, 0.15],
                                          [0.04, -0.01, 0.06, -0.34]]))
    ...
    def test_remove_samples_from_pcoa_data(self):
        ...
        # data
        out_h, out_c = keep_samples_from_pcoa_data(self.jk_coords_header,
                                                   self.jk_coords_data,
                                                   ['1', '3'])
        self.assertEqual(out_h, [['1', '3'], ['1', '3'], ['1', '3'],
                                 ['1', '3']])
        assert_almost_equal(out_c, [array([[1.2, 0.1, -1.2],
                                           [0.11, 5.33, -0.23]]),
                                    array([[-1.4, 0.05, 1.3],
                                           [0.14, 2.00, -1.11]]),
                                    array([[-1.5, 0.05, 1.6],
        ...
        self.assertEqual(out_headers, [['1', '3'], ['1', '3'], ['1', '3'],
                                       ['1', '3']])
