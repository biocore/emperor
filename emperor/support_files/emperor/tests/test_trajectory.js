...
$(document).ready(function() {
  ...
  /**
   *
   * Test that the trajectory object can be constructed without any problems and
  ...
  test("Test constructor exceptions", function(){
    ...
    throws(
      ...
      Error,
      'An error is raised if the number of coordinates does not correspond to'+
      ' the number of gradient points'
    );
  ...
  test("Test _generateInterpolatedCoordinates", function(){
    ...
    var expectedInterpolatedCoordinates = [{"x": 0, "y": 0, "z": 0},
      ...
      {"x": 3, "y": 3, "z": 3},
      {"x": 3, "y": 3, "z": 3},
      {"x": 3.3333333333333335, "y": 3.3333333333333335, "z": 3.3333333333333335},
  ...
  /**
   *
   * Test the trajectory object computes the number of points for a given delta
   * correctly.
   *
   */
  test('Test calculateNumberOfPointsForDelta', function(){
    ...
    trajectory = new TrajectoryOfSamples(sampleNames, gradientPoints,
                                         coordinates, 2, 10);
    equal(trajectory.calculateNumberOfPointsForDelta(3), 15, 'Number of points'+
          ' for delta is calculated correctly');
    equal(trajectory.calculateNumberOfPointsForDelta(8), 40, 'Number of points'+
          ' for delta is calculated correctly');
    equal(trajectory.calculateNumberOfPointsForDelta(7), 35, 'Number of points'+
          ' for delta is calculated correctly');
    equal(trajectory.calculateNumberOfPointsForDelta(11), 55, 'Number of '+
          'points for delta is calculated correctly');
    equal(trajectory.calculateNumberOfPointsForDelta(1), 5, 'Number of points'+
          ' for delta is calculated correctly');
  });
  ...
  test('Test linearInterpolation', function(){
    ...
    expectedResult = [{ "x": 0, "y": 0, "z": 0}, {"x": 0.2, "y": 0.2, "z": 0.2},
                      { "x": 0.4, "y": 0.4, "z": 0.4 },
  ...
  test('Test distanceBetweenPoints', function(){
    ...
    equal(result, Math.sqrt(3),'Distance between points is computed correctly');
  ...
  test('Test getSampleNamesAndDataForSortedTrajectories', function(){
    ...
    result = getSampleNamesAndDataForSortedTrajectories(mappingFileHeaders,
                                                            ...
                                                            'DOB');
    deepEqual(result, crunchedDataOneCategory, 'The data is computed correctly'+
